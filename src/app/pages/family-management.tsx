...
import { Plus, Users, UserCheck, Trash2, ChevronDown, ChevronUp, Shield, User, UserMinus } from "lucide-react";
...
export function FamilyManagement() {
  ...
  // --- NEW: DELETE PARENT ENTIRELY FROM DATABASE ---
  ...
  if (isLoading) return <div className="p-12 text-center text-green-600 animate-pulse font-medium">Loading Family Registry...</div>;
  ...
  return (
    <div className="p-6 max-w-7xl mx-auto space-y-8">
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 bg-white p-6 rounded-2xl shadow-sm border border-gray-100">
        <div>
          <h1 className="text-3xl font-bold text-gray-900 tracking-tight">Family Management</h1>
          <p className="text-gray-500 mt-1">Structure your community and link household heads.</p>
        </div>
        <div className="flex gap-3">
          <button onClick={() => setShowCreateParent(true)} className="flex items-center gap-2 px-5 py-2.5 bg-white border border-gray-200 text-gray-700 rounded-xl hover:bg-gray-50 transition-all font-medium shadow-sm">
            <UserCheck className="w-4 h-4 text-green-600" />
            New Parent
          </button>
          <button onClick={() => setShowCreateFamily(true)} className="flex items-center gap-2 px-5 py-2.5 bg-green-600 text-white rounded-xl hover:bg-green-700 transition-all font-medium shadow-md">
          ...
          return (
            <div 
              ...
              className={`bg-white rounded-2xl border transition-all duration-300 overflow-hidden ${
                isExpanded ? "ring-2 ring-green-500 shadow-lg scale-[1.02]" : "shadow-sm border-gray-200 hover:border-green-300"
              }`}
            >
              ...
              {!isExpanded && (
                <div className="px-6 pb-6 space-y-2">
                   <div className="flex items-center gap-2 text-sm text-gray-600">
                      ...
                      <span className="font-semibold">{parent?.name || "No Parent Assigned"}</span>
                   </div>
                </div>
              )}
                ...
                <div className="px-6 pb-6 pt-2 border-t border-gray-50 bg-gray-50/50 space-y-4">
                        ...
                        {familyMembers.map(member => (
                          <div key={member.id} className="flex items-center justify-between p-3 bg-white rounded-xl border border-gray-100 shadow-sm">
                            <div className="flex items-center gap-3">
                              {member.role === 'parent' ? <Shield className="w-4 h-4 text-green-600" /> : <User className="w-4 h-4 text-blue-500" />}
                              <div>
                                <p className="text-sm font-bold text-gray-800">{member.name}</p>
                                <p className="text-[10px] text-gray-400 font-mono capitalize">{member.role}</p>
                              </div>
                            </div>
                            
                            <div className="flex items-center gap-1">
                              <span className="text-[9px] bg-gray-100 px-2 py-0.5 rounded text-gray-500 hidden sm:block">ID: {member.id.slice(0, 5)}</span>
                  ...
                  <div className="flex gap-2 pt-2">
                    <button 
                      ...
                      className="flex-1 py-2 bg-white border border-green-200 text-green-700 rounded-lg text-xs font-bold hover:bg-green-50"
                    >
                      Assign Parent
                    </button>
                    <button 
          ...
          <div className="bg-white rounded-2xl p-8 w-full max-w-md shadow-2xl">
            ...
            <div className="space-y-4">
              ...
              <div className="flex gap-3 pt-4">
                <button onClick={() => setShowCreateParent(false)} className="flex-1 py-3 text-gray-500 font-bold hover:bg-gray-50 rounded-xl transition-all">Cancel</button>
                <button onClick={handleCreateParent} disabled={isSubmitting} className="flex-1 py-3 bg-green-600 text-white rounded-xl font-bold hover:bg-green-700 disabled:opacity-50 shadow-lg shadow-green-200 transition-all">
      ...
      {showAssignParent && (
        <div className="fixed inset-0 bg-black/60 backdrop-blur-sm flex items-center justify-center z-50 p-4">
          <div className="bg-white rounded-2xl p-6 sm:p-8 w-full max-w-md shadow-2xl">
            <h2 className="text-2xl font-bold text-gray-900 mb-6">Assign Family Head</h2>
            <div className="space-y-3 max-h-[400px] overflow-y-auto pr-2">
              {availableParents.length === 0 ? (
                <div className="text-center py-8 bg-gray-50 rounded-2xl border-2 border-dashed border-gray-200">
                  <p className="text-gray-400 font-medium">No unassigned parents.</p>
                </div>
              ) : (
                availableParents.map((parent) => (
                  <div key={parent.id} className="w-full flex items-center justify-between p-4 rounded-xl border border-gray-100 bg-white shadow-sm">
                    <div>
                      <p className="font-bold text-gray-800">{parent.name}</p>
                      <p className="text-[10px] text-gray-400 font-mono">UUID: {parent.id.slice(0, 12)}...</p>
                    </div>
                    {/* --- Assign and Delete buttons for unassigned parents --- */}
            ...
            <button onClick={() => { setShowAssignParent(false); setSelectedFamilyId(""); }} className="w-full mt-6 py-3 text-gray-500 font-bold hover:bg-gray-50 rounded-xl transition-all">Close</button>
          </div>
        </div>
      )}
      ...
      {showCreateFamily && (
        <div className="fixed inset-0 bg-black/60 backdrop-blur-sm flex items-center justify-center z-50 p-4">
          <div className="bg-white rounded-2xl p-8 w-full max-w-md shadow-2xl">
            <h2 className="text-2xl font-bold text-gray-900 mb-6">Register New Family</h2>
            <div className="space-y-4">
              <input type="text" value={newFamily.name} onChange={(e) => setNewFamily({ name: e.target.value })} placeholder="Family Name (e.g. The Abraham's)" className="w-full px-4 py-3 border border-gray-200 rounded-xl focus:ring-2 focus:ring-green-500 outline-none transition-all" />
              <div className="flex gap-3">
                <button onClick={() => setShowCreateFamily(false)} className="flex-1 py-3 text-gray-500 font-bold hover:bg-gray-50 rounded-xl transition-all">Cancel</button>
                <button onClick={handleCreateFamily} disabled={isSubmitting} className="flex-1 py-3 bg-green-600 text-white rounded-xl font-bold hover:bg-green-700 shadow-lg shadow-green-100 transition-all">
