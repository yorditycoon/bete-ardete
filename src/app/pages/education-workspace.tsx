...
import { 
  ...
  MapPin as MapIcon, Image as ImageIcon, Search, TrendingUp, Award, CheckCircle
} from "lucide-react";
...
export function EducationWorkspace() {
  ...
  useEffect(() => {
    ...
    const initializeComponent = async () => {
      try {
        ...
        await fetchData();
        channel = supabase.channel('edu-workspace-realtime').on('postgres_changes', { event: '*', schema: 'public' }, () => { fetchData(); }).subscribe();
  ...
  const handleSaveQuiz = async () => {
    ...
    try {
      const { data: quizData, error: quizError } = await supabase.from('quizzes').insert([{ title: quizTitle, book_id: quizBookId, quiz_type: quizType, assignment_id: quizType === 'weekly' ? selectedAssignment : null }]).select().single();
      if (quizError) throw quizError;
      const preparedQuestions = questions.map(q => ({ quiz_id: quizData.id, question_text: q.text, options: q.options, correct_option_index: q.correct }));
  ...
  const handleAddEvent = async () => {
    ...
    try {
      ...
      setMapPosition(null); setEventImage(null); setMapSearchQuery(""); fetchData();
  ...
  const handleDelete = async (table: string, id: string) => {
    ...
    try {
      ...
      if (table === 'assignments') await supabase.from('reading_progress').delete().eq('reading_id', id);

      const { error } = await supabase.from(table).delete().eq('id', id);
      if (error) throw error;
      toast.success("Deleted completely!");
      fetchData();
  ...
  if (isLoading) return <div className="flex flex-col items-center justify-center h-[30vh]"><Loader2 className="w-8 h-8 animate-spin text-green-600 mb-4" /><p className="text-gray-500 font-medium">Loading workspace...</p></div>;
              ...
              <Card className="border-green-200 shadow-sm bg-white">
                ...
                <CardContent className="space-y-6">
                  <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4 bg-green-50/50 p-4 rounded-xl border border-green-100">
                    ...
                    <div><Label className="text-xs text-black font-bold mb-1 block">Target Book</Label><Select onValueChange={setQuizBookId} value={quizBookId}><SelectTrigger className="bg-white border-green-200"><SelectValue placeholder="Select Book" /></SelectTrigger><SelectContent>{books.map(b => <SelectItem key={b.id} value={b.id}>{b.title}</SelectItem>)}</SelectContent></Select></div>
                    <div><Label className="text-xs text-black font-bold mb-1 block">Assessment Type</Label><Select onValueChange={setQuizType} value={quizType}><SelectTrigger className="bg-white border-green-200"><SelectValue placeholder="Type" /></SelectTrigger><SelectContent><SelectItem value="weekly">Weekly Quiz</SelectItem><SelectItem value="midterm">Midterm Exam</SelectItem><SelectItem value="final">Final Exam</SelectItem></SelectContent></Select></div>
                    {quizType === 'weekly' && quizBookId && (
                      <div className="lg:col-span-4 mt-2"><Label className="text-xs text-black font-bold mb-1 block">Link to Weekly Reading</Label><Select onValueChange={setSelectedAssignment}><SelectTrigger className="bg-white border-green-200"><SelectValue placeholder="Select which reading this quiz covers..." /></SelectTrigger><SelectContent>{filteredReadings.length === 0 && <SelectItem value="none" disabled>No readings found for this book</SelectItem>}{filteredReadings.map(r => <SelectItem key={r.id} value={r.id}>{r.week_title} - {r.chapters}</SelectItem>)}</SelectContent></Select></div>
                    )}
                  </div>
            ...
            <TabsContent value="events" className="space-y-6 animate-in fade-in duration-500">
              <Card className="border-green-200 shadow-sm bg-white">
                ...
                <CardContent className="space-y-5">
                  ...
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                    <div className="space-y-2"><Label className="text-xs text-black font-bold uppercase">Event Type</Label><Select value={newEvent.type} onValueChange={(v: any) => setNewEvent({...newEvent, type: v})}><SelectTrigger className="border-green-200"><SelectValue placeholder="Event Type" /></SelectTrigger><SelectContent><SelectItem value="gathering">Gathering</SelectItem><SelectItem value="meeting">Meeting</SelectItem><SelectItem value="event">Special Event</SelectItem></SelectContent></Select></div>
              ...
              <div className="space-y-3 mt-8">
                ...
                {events.map(event => {
                  ...
                  return (
                    <div key={event.id} className="flex justify-between items-center p-4 bg-white border border-green-200 rounded-xl hover:border-green-400 transition-colors shadow-sm">
